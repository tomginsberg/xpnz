...
let transactionFormatted = transactions
...
function cleanTransaction(item) {
  const totalAmount = item.amounts.reduce(
  ...
  return {
    ...
    normalizedWeights: item.weights.map(
      (amount) => (totalAmount * amount.weight) / totalWeight,
    ),
...
function dropMin(balances) {
  let minMember = Object.keys(balances).reduce((a, b) =>
...
export const ProductService = {
  async getProductsData(ledgerID) {
    console.log("Getting products data for ledger:", ledgerID);
    if (ledgerID === "trap") {
  ...
  deleteProduct(id) {
    ...
    transactionFormatted = transactionFormatted.filter(
  ...
  async getCategories() {
    return [
      ...new Set(
        transactionFormatted.map((transaction) => transaction.category),
      ),
    ];
  },
